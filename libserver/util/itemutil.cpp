...
{
    if(mBarcodeInserted.contains(barcode)) return;
...
void ItemUtil::insertStock(const QString &barcode, const QString &number, int type, float count, int flag, int link_id, bool recursive)
{
    insertStock(barcode, number, type, count, flag, link_id, QVariantMap(), recursive);
}

void ItemUtil::insertStock(const QString &barcode, const QString &number, int type, float count, int flag, QVariantMap data, bool recursive)
{
    insertStock(barcode, number, type, count, flag, 0, data, recursive);
}

void ItemUtil::insertStock(const QString &barcode, const QString &number, int type, float count, int flag, int link_id, QVariantMap data, bool recursive)
{
    if(mBarcodeInserted.contains(barcode)) return;
    mBarcodeInserted << barcode;
    ...
    if((itemFlag & ITEM_FLAG::CALCULATE_STOCK) != 0) {
        ...
        mDb->insert("stockcards", QVariantMap{{"barcode", barcode}, {"name", name}, {"type", type},
                                              {"number", number}, {"count", count}, {"stock", (lastStock + count)},
                                              {"flag", (itemFlag | flag)}, {"link_id", link_id}});
    }
    if(mLevel == 0) {
        mLevel++;
...
{
    if(mBarcodeUpdated.contains(barcode)) return;
    mBarcodeUpdated.append(barcode);
    DbResult res = mDb->where("barcode = ", barcode)->where("type = ", type)->where("link_id = ", link_id)->get("stockcards");
    if(res.isEmpty()) return;
    const QVariantMap &d = res.first();
    float oldCount = d["count"].toFloat();
    float diff = count - oldCount;
    if(diff == 0) return;
    mDb->where("barcode = ", barcode)->where("type = ", type)->
            where("link_id = ", link_id)->update("stockcards", QVariantMap{{"count", count}});
    mDb->exec(QString("UPDATE stockcards SET stock = stock + %1 WHERE barcode = '%2' AND created_at >= '%3'").arg(diff).arg(barcode).arg(d["created_at"].toString()));
