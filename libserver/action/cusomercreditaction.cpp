...
void CusomerCreditAction::afterInsert(const QVariantMap &data)
{
    ...
    QVariantMap d{{"date", QDateTime::currentDateTime()}, {"number", data["number"]},
                 {"type", TRANSACTION_TYPE::INCOME}, {"link_id", data["id"]},
                 {"link_type", TRANSACTION_LINK_TYPE::CUSTOMER_CREDIT}, {"transaction_total", 0},
                 {"user_id", data["user_id"]}, {"machine_id", data["machine_id"]},
                 {"bank_id", data["bank_id"]}, {"money_total", -data["credit"].toDouble()},
