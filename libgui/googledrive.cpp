...
static QString G_FILES{"https://www.googleapis.com/drive/v3/files?q=mimeType=\"application/octet-stream\" and trashed=false and \"%1\" in parents&orderBy=createdTime desc"};
static QString G_GET_FOLDER{"https://www.googleapis.com/drive/v3/files?q=mimeType=\"application/vnd.google-apps.folder\" and name=\"sultanbackup\" and trashed=false"};
...
void GoogleDrive::getFolder()
{
    ...
    connect(request, SIGNAL(finished(int,QNetworkReply::NetworkError,QByteArray)), SLOT(getFolderDone(int,QNetworkReply::NetworkError,QByteArray)));
    request->get(req);
}
...
void GoogleDrive::createFolder()
{
    ...
    auto request = new O2Requestor(mManager, mAuth, this);
    connect(request, SIGNAL(finished(int,QNetworkReply::NetworkError,QByteArray)), SLOT(createFolderDone(int,QNetworkReply::NetworkError,QByteArray)));
...
void GoogleDrive::uploadFileExec()
{
    ...
    auto request = new O2Requestor(mManager, mAuth, this);
    connect(request, SIGNAL(finished(int,QNetworkReply::NetworkError,QByteArray)), SLOT(uploadFileDone(int,QNetworkReply::NetworkError,QByteArray)));
...
void GoogleDrive::getFileExec()
{
    ...
    auto request = new O2Requestor(mManager, mAuth, this);
    connect(request, SIGNAL(finished(int,QNetworkReply::NetworkError,QByteArray)), SLOT(getFileDone(int,QNetworkReply::NetworkError,QByteArray)));
    request->get(req);
}
...
void GoogleDrive::downloadFile(const QString &fileid)
{
    ...
    auto request = new O2Requestor(mManager, mAuth, this);
    connect(request, SIGNAL(finished(int,QNetworkReply::NetworkError,QByteArray)), SLOT(downloadDone(int,QNetworkReply::NetworkError,QByteArray)));
